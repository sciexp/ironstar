...
## Task 4b — build-graph baseline-zero and the graph-drift regulator (superseded by Task 7)

This section records the build-graph drift regulator as a live, gating flake check. Task 7 (re-round 2026-06-14) retracted that regulator: `modules/checks/build-graph-invariants.nix` and `modules/checks/build-graph-baseline.json` were deleted and the snapshot was demoted to a non-gating committed record. The evidence below remains accurate as a historical account of the regulator's behavior while it existed, but the check surface it describes (27, including `build-graph-invariants`) no longer reflects `.#checks`; see the Task 7 record below for the 26-check end state.
...
### Check-surface arithmetic (superseded — see Task 7)

At task-4b time, `nix eval .#checks.aarch64-darwin --apply builtins.attrNames` (in a git-tracked copy that includes the now-deleted regulator) showed 27 names, adding `build-graph-invariants`. Task 7 (below) retracted that regulator, so the live surface is 26 — `build-graph-invariants` is absent:
`cargo-nix-lock-sync`, `dev-platform`, `gitleaks`, `ironstar`, `ironstar-analytics-infra-test`, `ironstar-analytics-test`, `ironstar-core-test`, `ironstar-docs`, `ironstar-docs-e2e`, `ironstar-docs-unit`, `ironstar-e2e`, `ironstar-event-bus-test`, `ironstar-event-store-test`, `ironstar-eventcatalog`, `ironstar-eventcatalog-e2e`, `ironstar-eventcatalog-unit`, `ironstar-session-store-test`, `ironstar-session-test`, `ironstar-shared-kernel-test`, `ironstar-test`, `ironstar-todo-test`, `ironstar-workspace-test`, `structure-package-set-invariant`, `treefmt`, `workspace-clippy`, `workspace-test`.
...
## Task 7 — build-graph regulator demotion (re-round 2026-06-14)

The task-4b `build-graph-invariants` regulator was retracted. The CI-churn and bot-commit surface it imposed on Renovate PRs outweighed its value, because workspace-member presence is already hard-gated by the per-member `*-test` checks.

- Deletions: `modules/checks/build-graph-invariants.nix` (the gating `runCommand`) and `modules/checks/build-graph-baseline.json` (the ceilings) are removed from the repository. The graph-drift regulator no longer exists in `.#checks`.
- Surface return: the live check surface returns to 26 = 15 prior + 11 per-member `*-test`. Authoritative enumeration `nix eval .#checks.x86_64-linux --apply builtins.attrNames` yields 26 names with `build-graph-invariants` absent (`builtins.attrNames` index of `build-graph-invariants` is `null`):
`cargo-nix-lock-sync`, `dev-platform`, `gitleaks`, `ironstar`, `ironstar-analytics-infra-test`, `ironstar-analytics-test`, `ironstar-core-test`, `ironstar-docs`, `ironstar-docs-e2e`, `ironstar-docs-unit`, `ironstar-e2e`, `ironstar-event-bus-test`, `ironstar-event-store-test`, `ironstar-eventcatalog`, `ironstar-eventcatalog-e2e`, `ironstar-eventcatalog-unit`, `ironstar-session-store-test`, `ironstar-session-test`, `ironstar-shared-kernel-test`, `ironstar-test`, `ironstar-todo-test`, `ironstar-workspace-test`, `structure-package-set-invariant`, `treefmt`, `workspace-clippy`, `workspace-test`.
- Snapshot survives as a non-gating record: the build-graph topology snapshot persists only as a committed, non-gating record at `modules/apps/build-graph-snapshot/snapshot.json`. It is regenerated on-demand via `just regenerate-build-graph-snapshot` or the `build-graph-snapshot` flake app (`nix run .#build-graph-snapshot`), and reviewed at human cadence rather than enforced on every PR. The determinism and topology evidence recorded under Task 4b and Task 5 above remains valid for the snapshot as a record; only its gating status changed.
...
`nix eval .#checks.aarch64-darwin --apply builtins.attrNames` -> the set is unchanged by the swap; after the Task 7 demotion the live surface is 26 names (`build-graph-invariants` absent):
`cargo-nix-lock-sync`, `dev-platform`, `gitleaks`, `ironstar`, `ironstar-analytics-infra-test`, `ironstar-analytics-test`, `ironstar-core-test`, `ironstar-docs`, `ironstar-docs-e2e`, `ironstar-docs-unit`, `ironstar-e2e`, `ironstar-event-bus-test`, `ironstar-event-store-test`, `ironstar-eventcatalog`, `ironstar-eventcatalog-e2e`, `ironstar-eventcatalog-unit`, `ironstar-session-store-test`, `ironstar-session-test`, `ironstar-shared-kernel-test`, `ironstar-test`, `ironstar-todo-test`, `ironstar-workspace-test`, `structure-package-set-invariant`, `treefmt`, `workspace-clippy`, `workspace-test`.
The `ironstar` check NAME and the count are stable across the swap; surface-back trigger #4 (check count/semantics change) does not fire.
...
The artifacts were amended to the as-built contract with no production-code logic touched: `spec.md` rewrote the test/clippy-gate requirement to the per-member `runTests` shape (933 passed / 5 ignored over 938 defined, offline `importCargoLock` clippy, check surface 26 = 15 prior + 11 per-member `*-test`) and reconciled the build-graph requirement to a non-gating committed record; `proposal.md` body and `design.md` were reconciled to one figure set (check surface 26 = 15 prior + 11 per-member `*-test`; 933/5/938 in place of the stale 911; canonical snapshot roots renamed to `packages.ironstar`/`packages.ironstar-release`).
