...
- [x] 1.1 Refactor the app to extract once into `logs/build-graph/raw/<cat>__<name>.json` for the 16 canonical roots, then project the committed snapshot from the persisted raw rather than re-extracting.
- [x] 1.2 Add `provenance.{cargo_nix_sha256, root_drv_paths_sha256}` to the committed snapshot.
- [x] 1.3 Gitignore `logs/build-graph/raw/`.
- [x] 1.4 Acceptance: the committed `snapshot.json` is byte-identical after the refactor (sha256 evidence before/after); all 16 raw files persist under `logs/build-graph/raw/`.
...
- [x] 2.1 Emit `logs/build-graph/edges.ndjson` from the persisted raw via the `inputs.drvs`-based `root_edges` projection; label it as the build-closure projection.
- [x] 2.2 Load `logs/build-graph/build-graph.duckdb` with the `nodes`, `edges`, and `root_membership` tables.
- [x] 2.3 Gitignore `logs/build-graph/edges.ndjson` and `logs/build-graph/build-graph.duckdb`.
- [x] 2.4 Acceptance: the app exits 0; the DuckDB fidelity query over `nodes` reproduces the committed snapshot's `dev_profile_crates_with_multiplicity=202`.
...
- [x] 3.1 Render the member-dag and crate-overview from the crate-DAG parse.
- [x] 3.2 Render the five rebuild cones from the build-closure projection (reverse-reachability, transitive-reduced, `rankdir=BT`, role-colored), SVG + PNG, PDF gated on the `dot -Tpdf` probe.
- [x] 3.3 Generate `index.md` stating, per artifact, which edge semantics it carries (build-closure projection vs crate-DAG parse).
- [x] 3.4 Gitignore `logs/build-graph/graph-viz/`.
- [x] 3.5 Acceptance: the member-dag, the crate-overview, and the five cones render with a generated index; each artifact and the index state their edge semantics.
...
- [x] 4.1 Add the `just build-graph-report` recipe wrapping the compendium app.
- [x] 4.2 Write the review runbook: at least four canned DuckDB queries plus a four-item gap checklist.
- [x] 4.3 Preserve the retired baseline ceilings as `reference/ceilings.json` (non-gating reference data).
- [x] 4.4 Acceptance: `just build-graph-report` dispatches; the runbook has at least four canned queries and a four-item gap checklist; the ceilings are preserved as non-gating reference, with no build-graph check on the check surface and no CI workflow invoking the compendium.
