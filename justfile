...
[group('cloudflare')]
cf-preview:
  bun run --filter '@ironstar/docs' preview

# Build and deploy the site to Cloudflare Workers
[group('cloudflare')]
cf-build-deploy: install
  bun run --filter '@ironstar/docs' deploy
...
docs-deploy-preview branch=`git branch --show-current`:
  ...
  echo "  Preview URL: https://b-${SAFE_BRANCH}-ironstar-docs.sciexp.workers.dev"
...
docs-deploy-production:
  ...
  if [ -n "$EXISTING_VERSION" ]; then
    ...
    "; then
      ...
      echo "  Production URL: https://ironstar.scientistexperience.net"
    else
      echo ""
...
# Deploy preview version with aliased preview URL for branch
[group('eventcatalog')]
eventcatalog-deploy-preview branch=`git branch --show-current`:
  #!/usr/bin/env bash
  set -euo pipefail
  cd packages/eventcatalog
  ...
  echo "  Full SHA: ${COMMIT_SHA}"
  echo "  Message: ${MESSAGE}"
  echo "  Preview URL: https://b-${SAFE_BRANCH}-ironstar-events.sciexp.workers.dev"

# Deploy to production (promote existing version or fallback to build+deploy)
[group('eventcatalog')]
eventcatalog-deploy-production:
  #!/usr/bin/env bash
  set -euo pipefail
  cd packages/eventcatalog
  ...
  if [ -n "$EXISTING_VERSION" ]; then
    ...
    "; then
      ...
      echo "  Full SHA: ${CURRENT_SHA}"
      echo "  Deployed by: ${DEPLOY_MSG}"
      echo "  Production URL: https://events.ironstar.scientistexperience.net"
    else
      echo ""
      echo "Failed to promote version ${EXISTING_VERSION}"
      echo "  Deployment was cancelled or failed"
...
# List recent versions
[group('eventcatalog')]
eventcatalog-versions limit="10":
  cd packages/eventcatalog && sops exec-env ../../vars/shared.yaml "bunx wrangler versions list --limit {{limit}}"

# List deployments
[group('eventcatalog')]
eventcatalog-deployments:
  cd packages/eventcatalog && sops exec-env ../../vars/shared.yaml "bunx wrangler deployments list"

# Tail live logs from Cloudflare Workers
[group('eventcatalog')]
eventcatalog-tail:
  cd packages/eventcatalog && sops exec-env ../../vars/shared.yaml "bunx wrangler tail"
...
# Build the queryable DuckDB build-graph report and rendered review artifacts.
...
sops-add-key:
  ...
  printf "Enter age key description (e.g., 'ironstar [dev|ci]'): "
...
[group('testing')]
test:
  bun run --filter '@ironstar/*' test

# Run tests in specific package
[group('testing')]
test-pkg package:
  bun run --filter '@ironstar/{{ package }}' test

# Run unit tests in docs
[group('testing')]
test-unit:
  bun run --filter '@ironstar/docs' test:unit

# Run E2E tests in docs
[group('testing')]
test-e2e:
  bun run --filter '@ironstar/docs' test:e2e

# Run vitest in watch mode
[group('testing')]
test-watch:
  bun run --filter '@ironstar/docs' test:watch

# Run playwright in UI mode
[group('testing')]
test-ui:
  bun run --filter '@ironstar/docs' test:ui

# Generate test coverage report
[group('testing')]
test-coverage:
  bun run --filter '@ironstar/docs' test:coverage
