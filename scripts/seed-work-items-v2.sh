#!/usr/bin/env bash
# Ironstar Work Items Seed Script v2
# Generated: 2025-12-17
# Updated: 2025-12-18 (Phase 1A-D integration)
...
#   2. Rust Workspace Integration (crane, rust-overlay, Cargo workspace, CI checks)
#   3. Process Compose Integration (flake integration, service orchestration)
#   4. Domain Layer (algebraic types, aggregates, signals)
#   5. Frontend Build Pipeline (Rolldown, PostCSS, Open Props, web components)
#   6. Event Sourcing Infrastructure (SQLite, redb, tokio broadcast, DuckDB)
#   7. Presentation Layer (axum, SSE, hypertext templates)
#   8. Example Application - Todo (complete demo)
#   9. Third-Party Library Integration (vega, sortable, lucide)
...
#
# Changes in v2:
#   - Epic 4 (CI/CD) absorbed into Epic 2 (Rust Workspace Integration)
#   - Added 8 new tasks for production readiness and DX improvements
#   - Enhanced task descriptions with concrete implementation details
#   - Fixed coarse dependency anti-patterns (Epic → Epic replaced with Task → Task)
#   - Added granular task dependencies for proper sequencing
#
# Usage: ./scripts/seed-work-items-v2.sh
#
# Prerequisites:
...
log_info "Creating ironstar work items (v2 with Phase 1A-D integration)..."
...
bd update "$TASK_RUST_FLAKE" --description "Create nix/modules/rust.nix importing rust-flake module. Configure:
...
TASK_CRATE_NIX=$(bd create "Set up per-crate crate.nix pattern for crane args" \
    -p 0 --parent "$EPIC_RUST" 2>&1 | extract_id)
bd update "$TASK_CRATE_NIX" --description "Create crate.nix files for each workspace crate defining crane-specific build arguments:
...
TASK_RUST_CHECKS=$(bd create "Add Rust checks to flake.checks for CI integration" \
    -p 0 --parent "$EPIC_RUST" 2>&1 | extract_id)
bd update "$TASK_RUST_CHECKS" --description "Create nix/modules/checks.nix defining perSystem.checks with:
...
bd dep add "$TASK_RUST_CHECKS" "$TASK_CRATE_NIX"

TASK_RUST_CI_MATRIX=$(bd create "Add Rust to CI matrix and extend inherited workflows" \
    -p 0 --parent "$EPIC_RUST" 2>&1 | extract_id)
bd update "$TASK_RUST_CI_MATRIX" --description "Extend template's .github/workflows/ci.yml inherited from typescript-nix-template with Rust jobs:
...
bd dep add "$TASK_RUST_CI_MATRIX" "$TASK_RUST_CHECKS"
...
bd update "$TASK_PC_FLAKE" --description "Create nix/modules/process-compose.nix importing process-compose-flake.flakeModule:
{
...
TASK_SERVICE_ORCH=$(bd create "Set up service orchestration (frontend bundler, cargo-watch)" \
    -p 0 --parent "$EPIC_PROCESS" 2>&1 | extract_id)
bd update "$TASK_SERVICE_ORCH" --description "Configure service startup order and dependencies in process-compose.yaml:
processes:
  db-init:
...
TASK_HOTRELOAD_TRIGGER=$(bd create "Configure cargo-watch to curl hotreload trigger on success" \
    -p 0 --parent "$EPIC_PROCESS" 2>&1 | extract_id)
bd update "$TASK_HOTRELOAD_TRIGGER" --description "Update backend process command in process-compose.yaml:
  backend:
...
bd dep add "$TASK_HOTRELOAD_TRIGGER" "$TASK_SERVICE_ORCH"
...
bd dep add "$TASK_PC_VERIFY" "$TASK_HOTRELOAD_TRIGGER"

###############################################################################
# EPIC 4: Domain Layer
###############################################################################
log_info "Creating Epic 4: Domain Layer..."
...
bd dep add "$EPIC_DOMAIN" "$TASK_CARGO_CHECK"
...
TASK_DOMAIN_TYPES=$(bd create "Define algebraic domain types and aggregate structure" \
    -p 0 --parent "$EPIC_DOMAIN" 2>&1 | extract_id)
bd update "$TASK_DOMAIN_TYPES" --description "Implement sum types for DomainEvent, Command, and aggregate states as Rust enums with serde serialization:
...
TASK_VALUE_OBJECTS=$(bd create "Implement value objects and smart constructors" \
    -p 0 --parent "$EPIC_DOMAIN" 2>&1 | extract_id)
bd update "$TASK_VALUE_OBJECTS" --description "Create validated value objects with smart constructor pattern:
#[derive(Debug, Clone, Serialize, Deserialize)]
...
TASK_AGGREGATES=$(bd create "Design aggregate root state machines" \
    -p 0 --parent "$EPIC_DOMAIN" 2>&1 | extract_id)
bd update "$TASK_AGGREGATES" --description "Model aggregate state machines using Rust enums:
enum TodoAggregate {
...
TASK_SIGNAL_CAMELCASE=$(bd create "Enforce camelCase convention for Datastar signal fields" \
    -p 0 --parent "$EPIC_DOMAIN" 2>&1 | extract_id)
bd update "$TASK_SIGNAL_CAMELCASE" --description "Apply #[serde(rename_all = \"camelCase\")] to all signal struct definitions:
...
bd dep add "$TASK_SIGNAL_CAMELCASE" "$TASK_SIGNAL_TYPES"

TASK_CMD_VALIDATION=$(bd create "Implement command validation pattern with Result types" \
    -p 0 --parent "$EPIC_DOMAIN" 2>&1 | extract_id)
bd update "$TASK_CMD_VALIDATION" --description "Create validation functions returning Result<ValidatedCommand, ValidationError>:
impl Command {
...
bd dep add "$TASK_CMD_VALIDATION" "$TASK_AGGREGATES"
...
###############################################################################
# EPIC 5: Frontend Build Pipeline
###############################################################################
log_info "Creating Epic 5: Frontend Build Pipeline..."
...
bd dep add "$EPIC_FRONTEND" "$TASK_RUST_CI_MATRIX"
...
TASK_ROLLDOWN=$(bd create "Configure Rolldown bundler with content-based hashing" \
    -p 0 --parent "$EPIC_FRONTEND" 2>&1 | extract_id)
bd update "$TASK_ROLLDOWN" --description "Create web-components/rolldown.config.ts:
...
TASK_CSS_LAYERS=$(bd create "Configure CSS cascade layers for predictable specificity" \
    -p 0 --parent "$EPIC_FRONTEND" 2>&1 | extract_id)
bd update "$TASK_CSS_LAYERS" --description "Add @layer declarations to web-components/styles/main.css:
...
bd dep add "$TASK_CSS_LAYERS" "$TASK_OPEN_PROPS"
...
bd dep add "$TASK_OPUI_COMPONENTS" "$TASK_CSS_LAYERS"
...
bd dep add "$TASK_TSRS_CONFIG" "$TASK_SIGNAL_TYPES"
...
TASK_MANIFEST_PARSE=$(bd create "Implement manifest.json parser for hashed filename resolution" \
    -p 0 --parent "$EPIC_FRONTEND" 2>&1 | extract_id)
bd update "$TASK_MANIFEST_PARSE" --description "Create AssetManifest struct parsing static/dist/manifest.json:
#[derive(Deserialize)]
...
bd dep add "$TASK_MANIFEST_PARSE" "$TASK_STATIC_DIST"
...
bd dep add "$TASK_RUST_EMBED" "$TASK_MANIFEST_PARSE"

TASK_WC_COMPONENTS=$(bd create "Create web-components/components/ directory for vanilla web components" \
    -p 1 --parent "$EPIC_FRONTEND" 2>&1 | extract_id)
bd update "$TASK_WC_COMPONENTS" --description "Set up web-components/components/ directory structure for vanilla web components. Create index.ts that exports/registers all components:
...
###############################################################################
# EPIC 6: Event Sourcing Infrastructure
###############################################################################
log_info "Creating Epic 6: Event Sourcing Infrastructure..."
...
TASK_SQLITE_ES=$(bd create "Implement SQLite event store with sqlx" \
    -p 0 --parent "$EPIC_EVENTSRC" 2>&1 | extract_id)
bd update "$TASK_SQLITE_ES" --description "Create SqliteEventStore struct implementing EventStore trait:
...
TASK_PROJECTION_TRAIT=$(bd create "Create Projection trait for read models" \
    -p 0 --parent "$EPIC_EVENTSRC" 2>&1 | extract_id)
bd update "$TASK_PROJECTION_TRAIT" --description "Define async trait with associated types:
#[async_trait]
...
TASK_PROJECTION_MGR=$(bd create "Implement ProjectionManager with in-memory state" \
    -p 0 --parent "$EPIC_EVENTSRC" 2>&1 | extract_id)
bd update "$TASK_PROJECTION_MGR" --description "Create generic ProjectionManager<P: Projection> wrapping Arc<RwLock<P::State>>:
...
TASK_SSE_KEEPALIVE=$(bd create "Implement SSE 15-second keep-alive comment stream" \
    -p 0 --parent "$EPIC_EVENTSRC" 2>&1 | extract_id)
bd update "$TASK_SSE_KEEPALIVE" --description "Add tokio::time::interval(Duration::from_secs(15)) to SSE stream merging:
...
bd dep add "$TASK_SSE_KEEPALIVE" "$TASK_EVENT_BUS"
...
TASK_SESSION_CLEANUP=$(bd create "Add session TTL cleanup background task" \
    -p 1 --parent "$EPIC_EVENTSRC" 2>&1 | extract_id)
bd update "$TASK_SESSION_CLEANUP" --description "Spawn tokio background task running every hour to delete expired sessions:
...
bd dep add "$TASK_SESSION_CLEANUP" "$TASK_REDB_SESSION"
...
Local refs: ~/projects/rust-workspace/axum"
bd dep add "$TASK_SESSION_EXTRACTOR" "$TASK_SESSION_CLEANUP"
...
###############################################################################
# EPIC 7: Presentation Layer
###############################################################################
log_info "Creating Epic 7: Presentation Layer..."
...
bd dep add "$TASK_APPSTATE" "$TASK_SESSION_CLEANUP"
...
TASK_SSE_FEED=$(bd create "Implement SSE feed endpoint with event replay" \
    -p 0 --parent "$EPIC_PRESENTATION" 2>&1 | extract_id)
bd update "$TASK_SSE_FEED" --description "Create async sse_feed handler:
...
bd dep add "$TASK_SSE_FEED" "$TASK_SSE_KEEPALIVE"

TASK_CMD_HANDLERS=$(bd create "Implement command POST handlers" \
    -p 0 --parent "$EPIC_PRESENTATION" 2>&1 | extract_id)
bd update "$TASK_CMD_HANDLERS" --description "Create POST handlers using ReadSignals extractor:
...
TASK_RENDERABLE_TRAIT=$(bd create "Implement RenderableToDatastar conversion trait" \
    -p 0 --parent "$EPIC_PRESENTATION" 2>&1 | extract_id)
bd update "$TASK_RENDERABLE_TRAIT" --description "Create extension trait for hypertext::Renderable:
...
TASK_GRACEFUL_SHUTDOWN=$(bd create "Implement graceful shutdown signal handling" \
    -p 0 --parent "$EPIC_PRESENTATION" 2>&1 | extract_id)
bd update "$TASK_GRACEFUL_SHUTDOWN" --description "Add tokio signal handling for SIGTERM/SIGINT:
...
bd dep add "$TASK_GRACEFUL_SHUTDOWN" "$TASK_ROUTER"
...
Local refs: ~/projects/rust-workspace/axum, ~/projects/rust-workspace/tokio"
bd dep add "$TASK_MAIN" "$TASK_GRACEFUL_SHUTDOWN"
...
###############################################################################
# EPIC 8: Example Application (Todo)
###############################################################################
log_info "Creating Epic 8: Example Application (Todo)..."
...
bd update "$TASK_TODO_DOMAIN" --description "Create src/domain/todo.rs with:
...
bd dep add "$TASK_TODO_DOMAIN" "$TASK_AGGREGATES"
...
###############################################################################
# EPIC 9: Third-Party Library Integration
###############################################################################
log_info "Creating Epic 9: Third-Party Library Integration..."
...
###############################################################################
# EPIC 10: Testing and Integration
###############################################################################
log_info "Creating Epic 10: Testing and Integration..."
...
###############################################################################
# EPIC 11: Documentation and Template
###############################################################################
log_info "Creating Epic 11: Documentation and Template..."
...
###############################################################################
echo ""
log_success "Work items creation complete (v2)!"
echo ""
log_info "Changes in v2:"
log_info "  - Epic 4 (CI/CD) absorbed into Epic 2 (Rust Workspace Integration)"
log_info "  - Added 8 new tasks: SSE_KEEPALIVE, CMD_VALIDATION, SESSION_CLEANUP, MANIFEST_PARSE,"
log_info "    HOTRELOAD_TRIGGER, SIGNAL_CAMELCASE, CSS_LAYERS, GRACEFUL_SHUTDOWN"
log_info "  - Enhanced task descriptions with concrete implementation details"
log_info "  - Fixed dependency anti-patterns (removed coarse Epic→Epic, added granular Task→Task)"
echo ""
bd stats
echo ""
