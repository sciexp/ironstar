...
pub async fn extract_catalog(name: &str) -> Result<PathBuf, AnalyticsInfraError> {
    let data = DuckLakeCatalogs::get(name).ok_or_else(|| {
        AnalyticsInfraError::analytics(format!("embedded catalog '{name}' not found"))
    })?;

    let catalog_dir = std::env::temp_dir().join("ironstar-catalogs");
    tokio::fs::create_dir_all(&catalog_dir).await.map_err(|e| {
        AnalyticsInfraError::analytics(format!("failed to create catalog directory: {e}"))
    })?;
    ...
    tokio::fs::write(&file_path, &data.data)
        .await
        .map_err(|e| {
            AnalyticsInfraError::analytics(format!("failed to write catalog '{name}': {e}"))
        })?;
...
/// # Errors
///
/// Returns `AnalyticsInfraError` only if the analytics service is unavailable.
/// Individual catalog failures are logged and skipped.
pub async fn attach_all(service: &DuckDBService) -> Result<Vec<String>, AnalyticsInfraError> {
    if !service.is_available() {
        return Err(AnalyticsInfraError::analytics(
...
async fn extract_and_attach(
    ...
    filename: &str,
    alias: &str,
) -> Result<(), AnalyticsInfraError> {
    let path = extract_catalog(filename).await?;

    let uri = format!(
        "ducklake:{}",
        path.to_str().ok_or_else(|| AnalyticsInfraError::analytics(
