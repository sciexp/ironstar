...
//! - Selecting the same catalog is idempotent (returns empty events)
//! - Selecting a different catalog while one is active is an error
//! - Refreshing metadata requires an active catalog
...
mod tests {
    ...
    fn idempotent_select_after_refresh_preserves_metadata() {
        ...
        // Select, refresh, then re-select same catalog → idempotent
        DeciderTestSpecification::default()
            .for_decider(catalog_decider())
            .given(vec![
                CatalogEvent::CatalogSelected {
