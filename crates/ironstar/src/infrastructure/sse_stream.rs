...
//!
//! ```rust,ignore
//! use ironstar::infrastructure::sse_stream::SseStreamBuilder;
...
/// ```rust,ignore
/// use zenoh::Session;
/// use ironstar::infrastructure::sse_stream::zenoh_to_sse_stream;
///
/// let subscriber = session.declare_subscriber("events/Todo/**").await?;
/// let sse_stream = zenoh_to_sse_stream(subscriber, |event: TodoEvent| {
///     Event::default()
...
///
/// ```rust,ignore
/// let stored_events = event_store.query_since_sequence(last_id).await?;
/// let replay = stored_events_to_stream(stored_events, |evt| {
///     Event::default()
///         .id(evt.sequence.to_string())
///         .data(render_html(&evt.event))
/// });
/// ```
