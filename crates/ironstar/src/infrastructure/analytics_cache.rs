...
///
/// ```rust,ignore
/// use ironstar::infrastructure::AnalyticsCache;
///
...
impl AnalyticsCache {
    ...
    /// # Errors
    ///
    /// Returns `InfrastructureError::cache` if the computation closure fails.
    ...
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = Result<Vec<u8>, InfrastructureError>>,
    {
    ...
    /// # Errors
    ///
    /// Returns `InfrastructureError::cache` if serialization fails.
    ...
    /// # Errors
    ///
    /// Returns `InfrastructureError::cache` if deserialization or validation fails.
    ...
    async fn get_or_insert_with_propagates_compute_error() {
        ...
        let result = cache
            .get_or_insert_with("err-key".to_string(), || async {
                Err(InfrastructureError::cache("compute failed"))
            })
            .await;
