...
use crate::key_expr::event_key_without_sequence;
...
impl EventBus for ZenohEventBus {
        ...
        E: Identifier + DeciderType + Serialize + Sync,
    {
        ...
        self.session
            ...
            .await
            .map_err(|e| EventBusError::event_bus(e.to_string()))?;
...
pub async fn open_embedded_session() -> Result<Session, EventBusError> {
    let config = zenoh_embedded_config();
    zenoh::open(config)
        .await
        .map_err(|e| EventBusError::event_bus(format!("failed to open zenoh session: {e}")))
}
