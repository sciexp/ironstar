...
//! # Schema
//!
//! See `EVENTS_MIGRATION_SQL` for the event store schema. Key features:
    ...
    E: DeserializeOwned + Clone,
{
    ...
    pub async fn fetch_all_events_by_type(
        &self,
        aggregate_type: &str,
    ) -> Result<Vec<(E, String)>, EventStoreError> {
        let rows = sqlx::query(
            r#"
    ...
    E: Identifier + EventType + DeciderType + IsFinal + Serialize + DeserializeOwned + Clone + Sync,
{
    ...
    pub async fn save_with_command(
        ...
        for event in events {
            ...
            {
                return Err(EventStoreError::optimistic_locking_conflict(
                    &aggregate_type,
                    &aggregate_id,
                ));
            }
...
/// SQL migration for the events table.
///
/// Embedded directly so tests can create the schema without depending on
/// the monolith's migrations directory.
pub const EVENTS_MIGRATION_SQL: &str = include_str!("events_migration.sql");

#[cfg(test)]
#[allow(clippy::expect_used, clippy::panic)]
mod tests {
    use super::*;
    use crate::error::EventStoreErrorKind;
    ...
    async fn create_test_pool() -> SqlitePool {
        ...
        // Run migration
        sqlx::query(EVENTS_MIGRATION_SQL)
            .execute(&pool)
            .await
