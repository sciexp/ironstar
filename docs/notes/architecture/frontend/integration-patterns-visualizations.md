...
| Topic | Document |
|-------|----------|
| Component API (properties, events, lifecycle) | `ds-echarts-integration-guide.md` |
| Backend integration patterns (SSE handlers, DuckDB) | `ds-echarts-backend.md` |
| Build/test setup (Rolldown, Vitest, mocking) | `ds-echarts-build-test.md` |
| TypeScript signal contracts (ts-rs generation) | `signal-contracts.md` |
| Frontend build pipeline (Rolldown vs esbuild) | `frontend-build-pipeline.md` |
...
When integrating hypertext templates with Datastar SSE, use the `RenderableToDatastar` helper trait defined in the hypertext section of `../core/architecture-decisions.md`:
...
For complete SSE streaming patterns including event replay and projection updates, see `../cqrs/sse-connection-lifecycle.md` and `../cqrs/event-replay-consistency.md`.
...
- Core integration patterns and decision framework: `integration-patterns.md`
