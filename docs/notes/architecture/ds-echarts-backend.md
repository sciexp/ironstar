...
- **Zenoh architecture**: `zenoh-event-bus.md` — Key expression patterns, embedded config, migration from tokio broadcast
- **Analytics cache**: `analytics-cache-architecture.md` — moka cache with TTL, Zenoh-based invalidation
- **SSE patterns**: `sse-connection-lifecycle.md` — Client subscription, reconnection resilience, Last-Event-ID
