...
The categorical structures in this document formalize domain concepts discovered through collaborative modeling:

- **Free monoid** (events) ← Orange stickies from EventStorming
- **Kleisli arrows** (commands) ← Blue stickies from EventStorming
- **Catamorphism** (aggregates) ← Yellow stickies from EventStorming
- **Adjunctions** (projections) ← Read model requirements from Domain Storytelling

See `discovery-and-specification.md` for how these structures emerge from collaborative sessions.
