...
This follows the pattern in `session-security.md` for session fixation prevention.
...
See `session-management.md` for complete cookie configuration.
...
- **Session security patterns**: `session-security.md` — Zenoh key expressions, rate limiting, fixation prevention
