...
# then dispatches on $1 to select which payload to deploy. Contrast with
# release-packages.nix, which uses the same interpolation form because it
# also dispatches between two nix-built derivation outputs.
{ ... }:
{
  perSystem =
    {
