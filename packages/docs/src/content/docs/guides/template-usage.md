...
description: How to fork and customize ironstar for your projects
---

This guide explains how to use ironstar as a starting point for your TypeScript monorepo projects.
...
```bash
nix flake init --template github:sciexp/ironstar
```

### Option 3: Manual fork

```bash
git clone https://github.com/sciexp/ironstar.git my-project
cd my-project
rm -rf .git
git init
git add .
git commit -m "chore: initial commit from ironstar"
```
...
**Template package:** `@ironstar/docs`
...
**For ironstar:**
- Package: `@ironstar/docs`
- Worker: `ironstar-docs`
- Route: `ironstar.scientistexperience.net`
...
```just
# Before
bun run --filter '@ironstar/docs' dev

# After
bun run --filter '@yourorg/my-package-name' dev
```

Search and replace `@ironstar/docs` with your package name.
...
- Check [GitHub Issues](https://github.com/sciexp/ironstar/issues)
