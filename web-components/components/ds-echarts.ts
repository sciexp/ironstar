...
export class DsEcharts extends LitElement {
  ...
  private initChart() {
    ...
    this.chart = echarts.init(this.chartContainer, themeArg, { renderer: this.renderer })
    ...
    if (this.isEventEnabled('lifecycle')) {
      this.dispatchEvent(this.createChartEvent('chart-ready', {
  ...
  private updateOption() {
    ...
    try {
      ...
      if (Object.keys(parsed).length > 0) {
        ...
        if (this.isEventEnabled('lifecycle')) {
          this.dispatchEvent(this.createChartEvent('chart-updated', {
            timestamp: Date.now(),
          }))
        }
      }
      ...
      if (this.isEventEnabled('lifecycle')) {
        this.dispatchEvent(this.createChartEvent('chart-error', {
          message: String(e),
    ...
    this.resizeObserver = new ResizeObserver(() => {
      ...
      this.resizeTimeout = window.setTimeout(() => {
        ...
        if (this.chartContainer && this.isEventEnabled('lifecycle')) {
          this.dispatchEvent(this.createChartEvent('chart-resized', {
            width: this.chartContainer.clientWidth,
            height: this.chartContainer.clientHeight,
          }))
        }
  ...
  private setupMediaQueryListener() {
    ...
    this.mediaQueryHandler = () => {
      ...
      if (this.theme === 'default') {
        this.chart.dispose()
        this.chart = echarts.init(this.chartContainer, null, { renderer: this.renderer })
        this.updateOption()
      }
    }
  ...
  private setupEventListeners() {
    ...
    if (this.isEventEnabled('mouse')) {
      ...
      for (const eventName of mouseEvents) {
        const handler = (params: any) => {
          ...
          this.dispatchEvent(this.createChartEvent(`chart-${eventName}`, sanitized))
        }
    ...
    if (this.isEventEnabled('hover')) {
      ...
      const hoverStartHandler = (params: any) => {
        ...
        lastHoverTime = now
        const sanitized = this.sanitizePayload(params)
        this.dispatchEvent(this.createChartEvent('chart-hover-start', sanitized))
      }
    ...
    if (this.isEventEnabled('component')) {
      ...
      const legendHandler = (params: any) => {
        const sanitized = this.sanitizePayload(params)
        this.dispatchEvent(this.createChartEvent('chart-legend-change', sanitized))
      }
      ...
      const selectHandler = (params: any) => {
        const sanitized = this.sanitizePayload(params)
        this.dispatchEvent(this.createChartEvent('chart-select-change', sanitized))
      }
  ...
  private sanitizePayload<T extends Record<string, unknown>>(payload: T): Partial<T> {
  ...
  private isEventEnabled(category: 'lifecycle' | 'mouse' | 'component' | 'hover'): boolean {
    return this.events.split(',').map(s => s.trim()).includes(category)
  }
